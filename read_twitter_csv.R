...
Tweetfile="Tweets_cleaned.csv"
Userfile="Users_cleaned.csv"
setwd("C:/Machine_Learning/")

# Import the data and look at it
tweets <- read.csv(Tweetfile)
users <- read.csv(Userfile)
...
str(users)
(UserVarNames<-names(users))
(UserNumColumns<-ncol(users))

#analyze the tweets and the user data

#plot retweet count vs. favorite count to look for correlations
ggplot(data = tweets, aes(x = retweet_count, y = favorite_count)) +
...
mean(tweets$retweet_count, na.rm = T)
sd(tweets$retweet_count, na.rm = T)
...
users %>%
  ggplot(aes(screen_name)) +
  geom_bar() + coord_flip() +
  labs(x = "Tweet Count",
       y = "Users",
       title = "Twitter users")

#plot followers for each user
ggplot(data = users, aes(x = followers_count, y = screen_name)) +
  geom_point() + ggtitle("Number of followers")


#plot followers vs. favorites count
ggplot(data = users, aes(x = followers_count, y = favourites_count)) +
  geom_point() + ggtitle("favorites vs. followers")

## mean and standard deviation of followers and favorites
nrow(users)
summary(users$followers_count)
mean(users$followers_count, na.rm = T)
sd(users$followers_count, na.rm = T)

summary(users$favourites_count)
mean(users$favourites_count, na.rm = T)
sd(users$favourites_count, na.rm = T)
